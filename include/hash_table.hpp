...
//    size_t capacity;
//
//    // Используем std::hash для вычисления хеша
//    size_t getHashCode(const K& key) const {
//        return std::hash<K>{}(key);
//    }
//
...
//        // Ищем ключ в цепочке
//        for (size_t i = 0; i < table.get(index).getSize(); ++i) {
//            auto& node = table.get(index).get(i);
//            if (node.isOccupied && getHashCode(node.key) == hash && node.key == key) {
...
//        }
//
//        // Ключ не найден, добавляем новый узел в цепочку
//        table.get(index).add(HashNode<K, V>(key, value));
//        ++elementCount;
//        checkAndResize();
//    }
...
//        for (size_t i = 0; i < table.get(index).getSize(); ++i) {
//            if (table.get(index).get(i).isOccupied &&
//                getHashCode(table.get(index).get(i).key) == hash &&
//                table.get(index).get(i).key == key) {
//                return table.get(index).get(i).value;
//            }
//        }
